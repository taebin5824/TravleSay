...
;(() => {
    ...
    // 초기값
    const initial = { loginId: "", email: "", phoneNumber: "" }
    ...
    function mapServerErrorToField(message) {
        ...
        console.log("[v0] Mapping server error:", message) // Added debug logging for error mapping
        if (message.includes("현재 비밀번호가 올바르지 않습니다")) return "currentPassword"
        if (message.includes("현재 비밀번호와 다른 비밀번호를 사용해 주세요")) return "password"
    ...
    // ====== 초기 데이터 로드 ======
    const axios = window.axios // Declare axios variable
    ...
    form?.addEventListener("submit", async (e) => {
        e.preventDefault()
        if (!pageLoaded) {
            showFormError("잠시만요. 사용자 정보를 불러오는 중입니다.")
            return
        }
        ...
        if (pwDirty && hasText(password) ^ hasText(passwordConfirm)) {
            showFieldError(
                hasText(password) ? "passwordConfirm" : "password",
                "비밀번호와 비밀번호 확인을 모두 입력해 주세요.",
            )
            btnSubmit.dataset.busy = "0"
            return
        }
        ...
        // 변경 없음
        if (
            !hasText(payload.loginId) &&
            !hasText(payload.password) &&
            !hasText(payload.email) &&
            !hasText(payload.phoneNumber)
        ) {
            showFormError("변경사항 없습니다.")
            btnSubmit.dataset.busy = "0"
            return
        }
    ...
    async function submitUpdate(payload, forceRelogin) {
        ...
        try {
            ...
            console.log("[v0] Server error message:", msg) // Added debug logging for server error
            const field = mapServerErrorToField(msg)
            console.log("[v0] Mapped to field:", field) // Added debug logging for field mapping
